...
- ✨ **智能语言服务器等待机制**：使用指数退避策略（200ms, 400ms, 800ms...），最多等待1分钟确保语言服务器就绪
- ✨ **装饰缓存系统**：切换标签时从缓存立即恢复注释，无需重新加载
- ✨ **手动刷新命令**：新增 `Show Notes: 刷新当前文件的注释` 命令
- 🎯 **自动缓存清理**：文档关闭时自动清理对应缓存，避免内存泄漏

### Changed

- 🔧 **改进注释提取逻辑**：
  - 正确处理代码块，避免提取代码内容
  - 遇到 `@param`、`@return` 等标记时立即停止提取
  - 更严格的过滤：跳过方法签名、类型声明等
  - 要求有效内容长度 > 3，避免提取无意义短词
- 📈 **优化性能**：
  - 懒加载初始化，只在打开支持的文件时才初始化资源
  - 防抖处理文档修改事件（500ms）
  - 并发处理行扫描，分批渲染装饰
- 🐛 **修复切换标签时注释消失的问题**
...
- 实现 `waitForLanguageServer()` 函数，智能检测语言服务器状态
- 实现 `decorationsCache` Map 缓存装饰结果
- 添加文档关闭事件监听器，自动清理资源
- 添加详细的调试日志，方便问题排查
- 优化 `extractJavadocFirstLine()` 函数，更准确提取方法文档
